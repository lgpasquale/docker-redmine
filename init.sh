...
if [ -z "${REDMINE_SERVER_NAME}" ]; then
    ...
    sed -i "s/\([[:space:]]*ServerName\).*/\1 ${REDMINE_SERVER_NAME}/g" /etc/apache2/sites-available/redmine-ssl.conf
fi

if [ -z "${REDMINE_SERVER_ALIAS}" ]; then
    sed -i "s/\([[:space:]]*ServerAlias\).*/\1 ${REDMINE_SERVER_ALIAS}/g" /etc/apache2/sites-available/redmine.conf
